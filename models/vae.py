...
class VAE(object):
	...
	def __init__(self, params):
		...
		# Encoder
		self.qz_x = [DenseLayer(n_hidden, n_in, hidden_nonlinear, w_init=w_init, device=device), 
					 GaussianLayer(n_embedding, n_hidden, w_init=w_init, device=device)]
		
		# Decoder
		self.px_z = [DenseLayer(n_hidden, n_embedding, hidden_nonlinear, w_init=w_init, device=device),
					 DenseLayer(n_out, n_hidden, T.sigmoid, w_init=w_init, device=device)]
...
class SSL_VAE(object):
	...
	def __init__(self, params):
		...
		# Encoder / Inference
		self.qy_x  = [DenseLayer(n_hidden, n_in, softplus, w_init=w_init, device=device), 
					  DenseLayer(n_class, n_hidden, softmax, w_init=w_init, device=device)]
		self.qz_yx = [DenseLayer(n_hidden, n_in+n_class, softplus, w_init=w_init, device=device), 
					  GaussianLayer(n_embedding, n_hidden, w_init=w_init, device=device)]
		
		# Decoder / Generative
		self.px_yz = [DenseLayer(n_hidden, n_class+n_embedding, softplus, w_init=w_init, device=device), 
					  DenseLayer(n_out, n_hidden, T.sigmoid, w_init=w_init, device=device)]
...
class AuxVAE(object):
    ...
    def __init__(self, params):
        ...
        # Encoder / Inference
        self.qa_x  = [DenseLayer(n_hidden, n_in, T.relu, w_init=w_init, device=device), 
        			  GaussianLayer(n_aux, n_hidden, w_init=w_init, device=device)]
        self.qz_ax = [DenseLayer(n_hidden, n_in+n_aux, T.relu, w_init=w_init, device=device), 
                      GaussianLayer(n_embedding, n_hidden, w_init=w_init, device=device)]
        
        # Decoder / Generative
        self.px_z  = [DenseLayer(n_hidden, n_embedding, T.relu, w_init=w_init, device=device), 
                      DenseLayer(n_out, n_hidden, T.sigmoid, w_init=w_init, device=device)]
        self.pa_xz = [DenseLayer(n_hidden, n_in+n_embedding, T.relu, w_init=w_init, device=device), 
                      GaussianLayer(n_aux, n_hidden, w_init=w_init, device=device)]
...
class MMDVAE(object):
    ...
    def __init__(self, params):
        ...
        # Decoder / Generative
        self.px_z = [DenseLayer(n_hidden, n_embedding, hidden_nonlinear, w_init=w_init, device=device), 
                     DenseLayer(n_in, n_hidden, T.sigmoid, w_init=w_init, device=device)]
